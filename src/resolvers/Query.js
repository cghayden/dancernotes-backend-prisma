...
const Query = {
  ...
  async studio(parent, args, ctx, info) {
    ...
    const studio = await ctx.db.query.studio({ where: { id: args.id } }, info)
    return studio
  },
  ...
  async searchStudios(parent, args, ctx, info) {
    if (!ctx.request.userId) {
      throw new Error('You must be logged in to do that')
    }
  ...
  async dancer(parent, args, ctx, info) {
    ...
    const dancer = await ctx.db.query.dancer({ where: { id: args.id } }, info)
    return dancer
  },
  ...
  async singleRoutine(parent, args, ctx, info) {
    ...
    if (routine) {
      const filteredDancers = routine.dancers.filter(
        (dancer) => dancer.parent.id === ctx.request.userId
      )
